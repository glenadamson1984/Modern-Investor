...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const row of parsedData) {
      if (Object.keys(row).length > 0 && Object.values(row).some(v => v !== null && v !== "")) {
        ...
        if (collectionName === "trades") {
          ...
          if (!row.date) {
            ...
            if (row.totalReturn !== undefined) processedRow.totalReturn = parseFloat(row.totalReturn) || 0;
            if (row.ytdReturn !== undefined) processedRow.ytdReturn = parseFloat(row.ytdReturn) || 0;
            if (row.yearlyReturn !== undefined) processedRow.yearlyReturn = parseFloat(row.yearlyReturn) || 0;
