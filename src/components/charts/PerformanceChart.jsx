    ...
    const fetchData = async () => {
      try {
        ...
        Object.keys(dateMap).forEach((dateKey) => {
          ...
          const platforms = ["ACC 212", "INV 212", "eToro", "HL"];
          ...
          if (totalReturns.length > 0) {
            dateEntry["Total_Total"] = parseFloat(
              (
                totalReturns.reduce((sum, v) => sum + v, 0) /
                totalReturns.length
              ).toFixed(2),
            );
          }
          ...
          if (ytdReturns.length > 0) {
            dateEntry["Total_YTD"] = parseFloat(
              (
